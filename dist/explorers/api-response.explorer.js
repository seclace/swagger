...
exports.exploreGlobalApiResponseMetadata = (definitions, metatype) => {
    ...
    const extraModels = Reflect.getMetadata(constants_2.DECORATORS.API_EXTRA_MODELS, metatype);
    if (extraModels) {
...
exports.exploreApiResponseMetadata = (definitions, instance, prototype, method) => {
    ...
    const extraModels = Reflect.getMetadata(constants_2.DECORATORS.API_EXTRA_MODELS, method);
    if (extraModels) {
        mapExtraModelsToSwaggerDefinitions(extraModels, definitions);
    }
    if (responses) {
...
const mapExtraModelsToSwaggerDefinitions = (models, definitions) => lodash_1.mapValues(models, model => api_parameters_explorer_1.exploreModelDefinition(model, definitions));
const mapResponsesToSwaggerResponses = (responses, definitions) => lodash_1.mapValues(lodash_1.mapValues(responses, response => {
    ...
    if (!(shared_utils_1.isFunction(type) &&
        ...
        if (isArray) {
            return Object.assign(Object.assign({}, response), { schema: {
                    type: 'array',
                    items: {
                        type: swaggerType
                    }
...
exports.toArrayResponseWithDefinition = (response, name) => (Object.assign(Object.assign({}, response), { schema: {
        type: 'array',
        items: {
            $ref: `#/definitions/${name}`
        }
    } }));
exports.toResponseWithDefinition = (response, name) => (Object.assign(Object.assign({}, response), { schema: {
        $ref: `#/definitions/${name}`
    } }));
