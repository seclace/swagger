...
const exploreApiReflectedParametersMetadata = (instance, prototype, method) => {
    ...
    const parameters = lodash_1.omitBy(lodash_1.mapValues(parametersWithType, (val, key) => (Object.assign(Object.assign({}, val), { in: mapParamType(key, consumes) }))), val => val.in === DEFAULT_PARAM_TOKEN || (val.name && val.in === 'body'));
    ...
    return lodash_1.flatMap(reflectedParameters, (param) => {
        ...
        return modelProperties.map(key => {
            const reflectedParam = Reflect.getMetadata(constants_2.DECORATORS.API_MODEL_PROPERTIES, prototype, key) ||
                {};
            return Object.assign(Object.assign(Object.assign({}, param), reflectedParam), { name: key });
        });
    }).filter(lodash_1.identity);
};
const transformToArrayModelProperty = (metadata, key, type) => {
    const model = Object.assign(Object.assign({}, metadata), { name: key, type: 'array', items: Object.assign({}, type) });
    if (metadata.enum !== undefined) {
        delete model.enum;
        model.items = Object.assign(Object.assign({}, model.items), { enum: metadata.enum });
    }
    return model;
};
exports.exploreModelDefinition = (type, definitions, existingNestedModelNames = []) => {
    ...
    const propertiesWithType = modelProperties.map(key => {
        ...
        if (metadata.isArray) {
            ...
            return Object.assign(Object.assign({}, metadata), { name: key, type: itemType });
        }
    });
...
const mapParametersTypes = parameters => parameters.map(param => {
    ...
    const { type } = param;
    const paramWithStringType = lodash_1.pickBy(Object.assign(Object.assign({}, param), { type: type && shared_utils_1.isFunction(type)
    ...
    if (paramWithStringType.isArray) {
        return Object.assign(Object.assign({}, paramWithStringType), { type: 'array', items: {
